...
#  # kind network should use MTU configured for the default network
#  local mtu="$(docker network inspect bridge -f '{{ index .Options "com.docker.network.driver.mtu" }}')"

  if [ -n "$existing_network_id" ] ; then
    # ensure the network is configured correctly
    local existing_network_config="$(docker network inspect $existing_network_id | jq -r '.[] | "\(.EnableIPv6),\(.IPAM.Config),\(.Options["com.docker.network.bridge.enable_ip_masquerade"])"')"
