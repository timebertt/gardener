...
func ReconcileSeedWebhookConfig(ctx context.Context, c client.Client, webhookConfig client.Object, namespace string, caBundle []byte) error {
	...
	if _, err := controllerutils.GetAndCreateOrStrategicMergePatch(ctx, c, webhookConfig, func() error {
		...
		return MergeWebhookConfig(webhookConfig, desiredWebhookConfig)
	}); err != nil {
...
// MergeWebhookConfig sets current.Webhooks to desired.Webhooks for all kinds and version of webhook configs.
func MergeWebhookConfig(current, desired client.Object) error {
