...
func (c *ControllerOptions) AddFlags(fs *pflag.FlagSet) {
	...
	fs.StringVar(&c.HostsFile, "hosts-file", c.HostsFile, "Overwrite hosts file to manipulate for populating DNSRecords.")
}
...
// Completed is just here for consistency but doesn't make sense at all.
func (c *ControllerOptions) Completed() *ControllerOptions {
	return c
}

// Apply sets the values of this ControllerOptions in the given AddOptions.
func (c *ControllerOptions) Apply(opts *AddOptions) {
	opts.Controller.MaxConcurrentReconciles = c.MaxConcurrentReconciles
	opts.HostsFile = c.HostsFile
}
