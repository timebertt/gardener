...
const ControllerName = "backupupload_controller"
...
// AddOptions are options to apply when adding the backupupload controller to the manager.
...
func AddToManagerWithOptions(mgr manager.Manager, opts backupoptions.AddOptions) error {
	return backupupload.Add(mgr, backupupload.AddArgs{
		Actuator:          newActuator(opts.ContainerMountPath, opts.BackupBucketPath),
		ControllerOptions: opts.Controller,
		Predicates:        backupupload.DefaultPredicates(opts.IgnoreOperationAnnotation),
		Type:              local.Type,
	})
}
